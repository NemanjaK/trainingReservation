...
const Term = (props) => {
  ...
  const enebleButtonHandling = (term) => {
    if (term.occupancy == 0 || expirationDate < todayDate || myDateReservation.includes(term.time.split(' ')[0])) {
    ...
    <>
      {props.terms.map((term, index) => (
        <div class="col-md-6">
          ...
          {adminReservationShown && <AdministratorReservation reservation={reservationHandling} />}
          <div class="timetable-item">
            ...
            <div class="timetable-item-main">
              ...
              <div class="timetable-item-name">{term.typeOfTraining}</div>
              {authorization == 'ROLE_USER' && enebleButtonHandling(term)}
